...
__constant__ BYTE AES_ShiftRowTab_Inv[16] = {0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3};
...
extern "C" void decrypt_cuda(aes_block* aes_block_array, BYTE* key, int keyLen, int block_number) {
    aes_block *cuda_aes_block_array;
    BYTE *cuda_key;

    cudaMalloc(&cuda_aes_block_array, block_number * sizeof(aes_block));
    cudaMalloc(&cuda_key, keyLen * sizeof(BYTE));

    cudaMemcpy(cuda_aes_block_array, aes_block_array, block_number * sizeof(aes_block), cudaMemcpyHostToDevice);
    cudaMemcpy(cuda_key, key, keyLen * sizeof(BYTE), cudaMemcpyHostToDevice);

    int thrdperblock = 256;
    dim3 ThreadperBlock(thrdperblock);
    dim3 BlockperGrid((block_number + thrdperblock - 1) / thrdperblock);

    AES_Decrypt<<<BlockperGrid, ThreadperBlock>>>(cuda_aes_block_array, cuda_key, keyLen, block_number);

    cudaMemcpy(aes_block_array, cuda_aes_block_array, block_number * sizeof(aes_block), cudaMemcpyDeviceToHost);
