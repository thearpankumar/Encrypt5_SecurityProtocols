#include <stdio.h>
#include <stdlib.h>
#include <cuda.h>

#define BYTE unsigned char

class aes_block {
public:
    BYTE block[16];
};

__constant__ BYTE AES_Sbox[] = {
    
};

__constant__ BYTE AES_ShiftRowTab[16] = {0,5,10,15,4,9,14,3,8,13,2,7,12,1,6,11};
__constant__ BYTE AES_xtime[256];

__device__ inline BYTE subBytes(BYTE in, const BYTE* sbox) {
    return sbox[in];
}

__device__ inline void AES_SubBytes(BYTE state[], const BYTE* sbox) {
    for (int i = 0; i < 16; i++) {
        state[i] = subBytes(state[i], sbox);
    }
}

__device__ inline void AES_AddRoundKey(BYTE state[], BYTE rkey[]) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= rkey[i];
    }
}

__device__ inline void AES_ShiftRows(BYTE state[], const BYTE* shifttab) {
    BYTE h[16];
    memcpy(h, state, 16);
    for (int i = 0; i < 16; i++) {
        state[i] = h[shifttab[i]];
    }
}

__device__ inline void AES_MixColumns(BYTE state[], const BYTE* xtime) {
    BYTE s0, s1, s2, s3, h;
    for (int i = 0; i < 16; i += 4) {
        s0 = state[i + 0]; s1 = state[i + 1]; s2 = state[i + 2]; s3 = state[i + 3];
        h = s0 ^ s1 ^ s2 ^ s3;
        state[i + 0] ^= h ^ xtime[s0 ^ s1];
        state[i + 1] ^= h ^ xtime[s1 ^ s2];
        state[i + 2] ^= h ^ xtime[s2 ^ s3];
        state[i + 3] ^= h ^ xtime[s3 ^ s0];
    }
}

__global__ void AES_Encrypt(aes_block aes_block_array[], BYTE key[], int keyLen, int block_number) {
    int global_thread_index = blockDim.x * blockIdx.x + threadIdx.x;

    if (global_thread_index < block_number) {
        BYTE block[16];
        for (int i = 0; i < 16; i++) {
            block[i] = aes_block_array[global_thread_index].block[i];
        }

        int l = keyLen, i;
        AES_AddRoundKey(block, &key[0]);

        for (i = 16; i < l - 16; i += 16) {
            AES_SubBytes(block, AES_Sbox);
            AES_ShiftRows(block, AES_ShiftRowTab);
            AES_MixColumns(block, AES_xtime);
            AES_AddRoundKey(block, &key[i]);
        }

        AES_SubBytes(block, AES_Sbox);
        AES_ShiftRows(block, AES_ShiftRowTab);
        AES_AddRoundKey(block, &key[i]);

        for (int i = 0; i < 16; i++) {
            aes_block_array[global_thread_index].block[i] = block[i];
        }
    }
}

extern "C" void encrypt_cuda(aes_block* aes_block_array, BYTE* key, int keyLen, int block_number) {
    aes_block *cuda_aes_block_array;
    BYTE *cuda_key;

    cudaMalloc(&cuda_aes_block_array, block_number * sizeof(aes_block));
    cudaMalloc(&cuda_key, keyLen * sizeof(BYTE));

    cudaMemcpy(cuda_aes_block_array, aes_block_array, block_number * sizeof(aes_block), cudaMemcpyHostToDevice);
    cudaMemcpy(cuda_key, key, keyLen * sizeof(BYTE), cudaMemcpyHostToDevice);

    int thrdperblock = 256;
    dim3 ThreadperBlock(thrdperblock);
    dim3 BlockperGrid((block_number + thrdperblock - 1) / thrdperblock);

    AES_Encrypt<<<BlockperGrid, ThreadperBlock>>>(cuda_aes_block_array, cuda_key, keyLen, block_number);

    cudaMemcpy(aes_block_array, cuda_aes_block_array, block_number * sizeof(aes_block), cudaMemcpyDeviceToHost);

    cudaFree(cuda_aes_block_array);
    cudaFree(cuda_key);
}